...
def update_connected_clients_file():
    try:
        os.makedirs(os.path.dirname(CONNECTED_FILE), exist_ok=True)
        with open(CONNECTED_FILE, 'w', encoding='utf-8') as f:
            json.dump(list(active_clients.keys()), f, indent=2)
    except Exception as e:
...
def connect():
    ...
    try:
        ...
        # ---- TCP Keepalive (prevents SSL EOF and disconnections) ----
...
def execute_query():
    ...
    try:
        ssl_sock.send(query.encode())
        response = ssl_sock.recv(16384).decode()
